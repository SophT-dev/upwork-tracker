// ============================================================
// Upwork Competitor Profiler — Bookmarklet Source
//
...
//
// SETUP:
// 1. Deploy upwork-competitors.gs as a Web App (see README)
// 2. Paste your Web App URL below
// 3. Minify this file and add "javascript:" prefix for bookmarklet
//
// USAGE: Visit any Upwork freelancer profile page
// (upwork.com/freelancers/~...) and click the bookmarklet.
// ============================================================
...
(function () {
  ...
  function scanBody(pattern) {
    var m = document.body.innerText.match(pattern);
    return m ? m[0].trim() : '';
  }

  function scanBodyGroup(pattern, group) {
    var m = document.body.innerText.match(pattern);
  ...
  if (url.indexOf('/freelancers/') === -1 && url.indexOf('/agencies/') === -1) {
    alert('This bookmarklet only works on Upwork freelancer profile pages.\n\nNavigate to a profile (upwork.com/freelancers/~...) and try again.');
    return;
  }
  ...
  // Headline (service title below name)
  var headline = get([
  ...
  // Total Earnings
  var totalEarnings = get([
    '[data-test="total-earnings"]',
    '[data-test="earnings-badge"]',
    '[class*="earnings"]'
  ]);
  if (!totalEarnings) {
  ...
  // Description / About bio (first 600 chars)
  var description = get([
  ...
  function extractWorkJobs() {
    var results = [];
    // Try various selectors for job cards in work history
    var cardSelectors = [
    ...
    for (var c = 0; c < Math.min(cards.length, 3); c++) {
      ...
      // Review quote: italic or blockquote text
  ...
  var portfolioItems = '';
  var portSelectors = [
    '[class*="portfolio-item"] h3',
    '[class*="portfolioItem"] h3',
    '[class*="portfolio"] h3',
    '[class*="Portfolio"] h3',
  ...
  function extractTestimonials() {
    ...
    for (var ts2 = 0; ts2 < testSelectors.length; ts2++) {
      var testEls = document.querySelectorAll(testSelectors[ts2]);
      if (testEls.length > 0) {
        Array.from(testEls).slice(0, 2).forEach(function(el) {
  ...
  var testimonial1 = testimonials[0] || '';
  var testimonial2 = testimonials[1] || '';
  ...
  function extractCertifications() {
    ...
    for (var cs = 0; cs < certSelectors.length; cs++) {
      var certEls = document.querySelectorAll(certSelectors[cs]);
      if (certEls.length > 0) {
        Array.from(certEls).forEach(function(el) {
          var lines = (el.innerText || '').trim().split('\n').map(function(s) { return s.trim(); }).filter(Boolean);
          if (lines.length >= 1) {
            var certName = lines[0] || '';
            var provider = lines[1] || '';
            var date = lines[2] || '';
            if (certName) results.push([certName, provider, date].filter(Boolean).join(' | '));
          }
        });
        if (results.length > 0) break;
      }
  ...
  function extractEmployment() {
    ...
    var empSelectors = [
      ...
      '[class*="employment"] li',
      '[class*="Employment"] li',
      '[data-test*="employment"]'
    ];
    for (var es = 0; es < empSelectors.length; es++) {
      var empEls = document.querySelectorAll(empSelectors[es]);
      if (empEls.length > 0) {
        Array.from(empEls).forEach(function(el) {
          var lines = (el.innerText || '').trim().split('\n').map(function(s) { return s.trim(); }).filter(Boolean);
          if (lines.length >= 2) {
  ...
  var employmentHistory = extractEmployment();
  ...
  function extractOtherExperiences() {
    ...
    var otherSelectors = [
      ...
      '[class*="OtherExperience"]',
      '[data-test*="other-experience"]'
    ];
    for (var os = 0; os < otherSelectors.length; os++) {
      var otherEls = document.querySelectorAll(otherSelectors[os]);
      if (otherEls.length > 0) {
        Array.from(otherEls).forEach(function(el) {
          var lines = (el.innerText || '').trim().split('\n').map(function(s) { return s.trim(); }).filter(Boolean);
          if (lines.length >= 1) {
            var heading = lines[0];
            var body = lines.slice(1).join(' ').substring(0, 100);
            results.push(body ? heading + ' — ' + body : heading);
          }
        });
        if (results.length > 0) break;
      }
  ...
  var otherExperiences = extractOtherExperiences();
  ...
  style.id = 'upc-style';
  style.textContent = [
    '#upc-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.5);z-index:2147483647;display:flex;align-items:flex-start;justify-content:flex-end;font-family:Arial,sans-serif;font-size:13px;line-height:1.5}',
    ...
    '#upc-modal h2{font-size:14px;margin:0 0 4px;color:#14a800;font-weight:700}',
    '#upc-headline{font-size:12px;color:#555;margin:0 0 12px;font-style:italic;line-height:1.4}',
    '.upc-row{display:flex;justify-content:space-between;align-items:center;padding:4px 0;border-bottom:1px solid #f0f0f0;font-size:12px}',
    ...
    '#upc-save:disabled{background:#999;cursor:default}',
    ...
    '#upc-status{text-align:center;margin-top:8px;font-weight:700;font-size:12px;min-height:16px;color:#14a800}'
  ].join('');
  ...
  var html = '<div id="upc-modal">' +
    '<h2>🕵️ Competitor Profile</h2>' +
    ...
    row('Certs', certCount > 0 ? certCount + ' certs captured' : '') +
  ...
  document.getElementById('upc-cancel').addEventListener('click', function () {
    overlay.remove();
  });
  ...
  document.getElementById('upc-save').addEventListener('click', function () {
    ...
    if (WEB_APP_URL === 'YOUR_COMPETITORS_SCRIPT_URL') {
